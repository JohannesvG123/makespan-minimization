...
def run_all():
    ...
    os.system(f"{files_subset} >> logstt/instances.txt")
    inst = open(f"logstt/instances.txt", 'a')
    inst.write(f"{files_subset.__len__()}{files_subset}\n")
    inst.close()

    configs = ["two-job-random-swap,improvement,max"]

    for timeout_after in [150]:
        for num_solutions in [5, 50, 500, 5000]:
            for config in configs:
                for num_threads in [1, 4, 8]:
                    ...
                    name = f"diff,{num_threads}-threads,{num_solutions}-sol,{timeout_after}s-timeout,{config}"

                    prog = open(f"logstt/progress.txt", 'a')
                    ...
                    logs = open(f"logstt/logs_{s}.txt", 'a')
                    ...
                    for file in files_subset:
                        ...
                        # os.system(f"{mm}--path ./benchmarks/{file} {args} --measurement >> logstt/logs_{s}.txt 2>&1 &")  # --write --write-separate-files DAS BRAUCHT MAN FÜR WINDOWS!
                    ...
                    print(f"generated logs are written in logstt/logs_{s}.txt")
